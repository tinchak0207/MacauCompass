...
export const fetchMarketData = async (): Promise<MarketStats> => {
  ...
  try {
    const companyResponse = await Taro.request({
      url: COMPANY_API_URL,
      method: 'POST',
      header: {
    ...
    if (companyResponse.statusCode === 200) {
      companyData = companyResponse.data as CompanyApiResponse
    } else {
      console.warn('Company API returned non-OK status, using fallback.')
      companyData = getMockCompanyData()
    }
  } catch (error) {
    console.warn('Company API network error, using fallback.', error)
    companyData = getMockCompanyData()
  }

  try {
    const trademarkResponse = await Taro.request({
      url: TRADEMARK_CSV_URL,
      method: 'GET'
    })

    if (trademarkResponse.statusCode === 200) {
      const csvText = trademarkResponse.data as string
  ...
  const sortedCompanies = (companyData.values || []).sort((a, b) =>
    b.periodString.localeCompare(a.periodString)
  )
